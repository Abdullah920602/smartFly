...
const FlightDetails = () => {
  ...
  const fetchFlightDetails = async () => {
    try {
      setLoading(true);
      console.log('Loading flight details from backend API...');
      console.log('Looking for flight ID:', flightId);
      
      // Try to get flight from API first
      const response = await flightService.getFlightById(flightId);
      const foundFlight = response.flight;
      
      if (foundFlight) {
        console.log('Found flight from API:', foundFlight);
        setFlight(foundFlight);
      } else {
        // Flight not found in API
        console.log('Flight not found in API');
        setFlight(null);
      }
    } catch (error) {
      console.error('خطأ في جلب تفاصيل الرحلة:', error);
      // Set null on error
      console.log('Error occurred, flight not found');
      setFlight(null);
    } finally {
      setLoading(false);
    }
