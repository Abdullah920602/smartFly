...
const Flights = () => {
  ...
  const fetchFlights = async () => {
    try {
      setLoading(true);
      console.log('Loading flights from backend API...');
      
      // Try to get flights from API first
      const response = await flightService.getFlights();
      const flightsData = response.flights || [];
      
      if (flightsData.length > 0) {
        console.log('Flights loaded from API:', flightsData.length);
        setFlights(flightsData);
        setFilteredFlights(flightsData);
        setFlightCounts(calculateFlightCounts(flightsData));
      } else {
        // No flights in API
        console.log('No flights in API');
        setFlights([]);
        setFilteredFlights([]);
        setFlightCounts(calculateFlightCounts([]));
      }
    } catch (error) {
      console.error('Error fetching flights from API:', error);
      // Set empty array on error
      console.log('Error occurred, setting empty flights');
      setFlights([]);
      setFilteredFlights([]);
      setFlightCounts(calculateFlightCounts([]));
    } finally {
